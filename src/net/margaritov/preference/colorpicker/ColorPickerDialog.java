...
public class ColorPickerDialog extends Dialog implements
    ...
    private final int mXOSPColor;
    private int mNewColorValue;

    private static final int PALETTE_XOSP  = 0;
    ...
    public ColorPickerDialog(Context context, int theme, int initialColor,
            int androidColor, int xospColor) {
        super(context, theme);

        mInitialColor = initialColor;
        mAndroidColor = androidColor;
        mXOSPColor = xospColor;
    ...
    private void setUp() {
        ...
        if (mAndroidColor != 0x00000000 && mXOSPColor != 0x00000000) {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        if (item.getItemId() == R.id.xosp_flat_palette) {
            setPalette(PALETTE_XOSP);
            mColorTransitionAnimator.start();
            return true;
            ...
            return true;
        } else if (item.getItemId() == R.id.reset_xosp) {
            mColorPicker.setColor(mXOSPColor, true);
            return true;
        }
        return false;
    }
    ...
    private int getPalette() {
        return Settings.System.getInt(mResolver,
                Settings.System.COLOR_PICKER_PALETTE, PALETTE_XOSP);
    }
    ...
    private int getPanelViewButtonColor(int pallete, int index) {
        TypedArray ta;
        if (pallete == PALETTE_XOSP) {
            ta = mResources.obtainTypedArray(R.array.color_picker_xosp_flat_palette);
